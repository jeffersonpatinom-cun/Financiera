/* ----------------------------------------------------------------------------
   SINCRONIZADO CON PRODUCCION el 2026-09-02 (dump de la definicion viva en
   172.16.1.33 / CUN_REPOSITORIO). Escribe Financiera.Cartera_Meta_Comercial_Historico
   y Financiera.Cartera_Meta_Comercial_Snapshot_Mensual.
   Este es el archivo canonico: desplegar SIEMPRE desde aqui.
       driver.py deploy USP_Foto_Meta_Comercial_Mensual.sql

   Incluye (respecto de la version que este archivo tenia el 2026-08-25, que se
   habia quedado atras y cuya redeployada habria revertido todo lo de abajo):
     * PASO 2.5  backup mensual previo al refresco, con auto-reparacion de esquema.
     * PASO 5a   refresco COMPLETO de las columnas vivas (saldos, GR*, contacto),
                 armado por nombre desde la interseccion tabla ∩ #SRC.
     * #PROM     PRO_ACUMULADO de SINU.SRC_HIS_ACADEMICA + SRC_ALUM_PERIODO,
                 llave SOLO identificacion (reemplaza V_DEMOGRAFICO_ESTUDIANTE_VIG).
     * #MOODLE   NULLIF(lastaccess, 0): 0 es "nunca ingreso", no 1970-01-01.
     * PERIODO_CAL colapsado con MIN(fec_inicio)/MAX(fec_fin) por cod_periodo.

   NO tiene DELETE ni TRUNCATE sobre el historico: la tabla solo crece (UPDATE
   concatena la marca del mes, INSERT para credito nuevo, credito pagado se deja
   intacto porque no esta en #SRC).
   Las variantes *_OBSOLETO_* de este repo NO reflejan produccion; no desplegarlas.

   Ultima corrida verificada: 2026-09-01 07:40, 858 s, OK.
     Nuevos 15.405 | acumulados 40.864 | backup 65.456 filas.
   Validacion descriptiva: analisis_meta_septiembre_2026.sql
   ---------------------------------------------------------------------------- */
/* ============================================================================
