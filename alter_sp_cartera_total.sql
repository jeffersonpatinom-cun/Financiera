...
BEGIN
    ...
    BEGIN TRY
        ...
        BEGIN
            SELECT TOP 0
                ...
                CT.NOM_SECCIONAL, CT.MODALIDAD, CT.CICLO,
                CT.MARCA_ACADEMICA, CT.ESTADO_ALUMNO
        ...
        -- Agregar columnas academicas (desde Cartera_Total) si no existen
        IF NOT EXISTS (SELECT 1 FROM sys.columns
            WHERE object_id = OBJECT_ID('Financiera.Cartera_Gestion') AND name = 'NOM_SECCIONAL')
            ALTER TABLE Financiera.Cartera_Gestion ADD NOM_SECCIONAL VARCHAR(MAX) NULL;
        IF NOT EXISTS (SELECT 1 FROM sys.columns
            WHERE object_id = OBJECT_ID('Financiera.Cartera_Gestion') AND name = 'MODALIDAD')
            ALTER TABLE Financiera.Cartera_Gestion ADD MODALIDAD VARCHAR(MAX) NULL;
        IF NOT EXISTS (SELECT 1 FROM sys.columns
            WHERE object_id = OBJECT_ID('Financiera.Cartera_Gestion') AND name = 'CICLO')
            ALTER TABLE Financiera.Cartera_Gestion ADD CICLO VARCHAR(MAX) NULL;
        IF NOT EXISTS (SELECT 1 FROM sys.columns
            WHERE object_id = OBJECT_ID('Financiera.Cartera_Gestion') AND name = 'MARCA_ACADEMICA')
            ALTER TABLE Financiera.Cartera_Gestion ADD MARCA_ACADEMICA VARCHAR(50) NULL;
        IF NOT EXISTS (SELECT 1 FROM sys.columns
            WHERE object_id = OBJECT_ID('Financiera.Cartera_Gestion') AND name = 'ESTADO_ALUMNO')
            ALTER TABLE Financiera.Cartera_Gestion ADD ESTADO_ALUMNO VARCHAR(MAX) NULL;
        ...
        INSERT INTO Financiera.Cartera_Gestion
            (PERIODO, TIPO_CLIENTE, NOMBRE_TIPO_CLIENTE, IDENTIFICACION,
             FEC_NAC, GENDER, DIRECCION_CASA, EMAIL, TEL_CASA,
             TEL_CELULAR, WHATSAPP, PAIS, DEPARTAMENTO, CLIENTE,
             NOMBRE, LINEA, TIPO_DOCUMENTO, DOCUMENTO, NUMERO_CREDITO,
             FECHA, FECHA_VENCIMIENTO, CENTRO_COSTO, NOMBRE_CENTRO,
             FONDO, NOMBRE_FONDO, NOMBRE_CONCEPTO, NOMBRE_CAUSA,
             VALOR_ORIGINAL, CORRIENTE, GR1A30, GR31A60, GR61A90,
             GR91A120, GR121A150, GR151A360, GR360MAS, TOTAL,
             CODIGO_CONTABLE, DESCRIPCION,
             NOM_UNIDAD, NUEVO, SEMESTRE, PROMEDIO,
             ultimoaccesoplataformlimpio, NRO_CUOTA,
             NOM_SECCIONAL, MODALIDAD, CICLO, MARCA_ACADEMICA, ESTADO_ALUMNO)
        SELECT
            C.PERIODO, C.TIPO_CLIENTE, C.NOMBRE_TIPO_CLIENTE, C.IDENTIFICACION,
            C.FEC_NAC, C.GENDER, C.DIRECCION_CASA, C.EMAIL, C.TEL_CASA,
    ...
    -- VALIDACION fuera de transaccion (persiste aunque haya ROLLBACK)
